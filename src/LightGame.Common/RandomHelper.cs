...
{
    /// <summary>
    /// link: https://github.com/BenMakesGames/RandomHelpers
    /// </summary>
    public static class RandomHelper
    {
        private static int _minPercent = 1;
        ...
        public static int Roll(int rolls, int sides)
        {
            ...
            for (int i = 0; i < rolls; i++)
                total += Random.Shared.Next(sides);
        ...
        /// <returns></returns>
        public static T Next<T>(IReadOnlyList<T> list)
        {
        ...
        /// <returns></returns>
        public static string NextString(string allowedCharacters, int length)
        {
            return NextString(allowedCharacters.ToCharArray(), length);
        }
        ...
        /// <param name="length">The length of string to generate.</param>
        /// <returns></returns>
        public static string NextString(IList<char> allowedCharacters, int length)
        {
            char[] buffer = new char[length];

            for (int i = 0; i < length; i++)
                buffer[i] = allowedCharacters[Random.Shared.Next(allowedCharacters.Count)];
        ...
        /// <param name="rng"></param>
        /// <returns></returns>
        public static bool NextBool(double rate = 0.5d)
        {
        ...
        /// <returns></returns>
        public static int NextPercentBonus(int baseAmount, float percentModifier)
        {
            ...
            if (Random.Shared.Next() < chanceOfOneMore)
            {
                if (percentModifier < 0)
                    returnedAmount--;
                else
